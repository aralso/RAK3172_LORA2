...
uint8_t test1;
uint8_t cpt_g_tx_state;
uint8_t prec_g_tx_state;
...
void configure_radio_parameters(void)
{
	...
	// création de nodes fictifs
		...
		nodes[2].class = 2;
		nodes[2].nb_recus = 0;
		nodes[2].nb_envoyes = 0;
		nodes[2].nb_err = 0;
		nodes[2].latestRssi = 0;
		nb_nodes=3;
...
void lora_tx_state_step(void)
{
    ...
    else // le statut change
    {
    	cpt_g_tx_state = 0;
    	prec_g_tx_state = g_tx_state;
    }
    ...
    case TX_SENDING: {
       ...
       /* SUBGRF_SetDioIrqParams(  IRQ_TX_DONE | IRQ_TX_TIMEOUT,
            IRQ_TX_DONE | IRQ_TX_TIMEOUT,  IRQ_RADIO_NONE, IRQ_RADIO_NONE);   */
        lora_send_packet(&tx);
        break;
    }
    case TX_SENT: {
...
void info_Node(uint8_t id, uint8_t cpt)
{
	uint16_t pos;
	uint16_t size=0;
	uint8_t ret = mess_lora_cherche_size(id, cpt, &pos, &size);
	LOG_INFO("Info mess node:%i cpt:%i ret:%i pos:%i size:%i", id, cpt, ret, pos, size);
}
...
void lecture_Nodes(void)
{
	...
	LOG_INFO("queue0 H:%i T:%i  queue1 H:%i T:%i  queue2 H:%i T:%i", lora_head[0], \
			lora_tail[0], lora_head[1], lora_tail[1], lora_head[2], lora_tail[2]);
}
...
//LOG_INFO("lg:%i param:%i, head:%i tail:%i", mess->length, mess->param, lora_head, lora_tail);
	...
	//osDelay(100);
	LOG_INFO("enqueuelora:head:%d tail:%d mess:%s len:%i dest:%c", lora_head[q_id], lora_tail[q_id], mess->data, mess->length, mess->dest);
    //osDelay(100);
   	//UART_SEND("Send5\n\r");
...
// cpt : position du Xe message
// pos : sa position
// size_tot : tot size des messages du dest
// return 0:mess existent 1:pas de message 2-3:erreur
uint8_t mess_lora_cherche_size(uint8_t node_id, uint8_t cpt, uint16_t* pos, uint16_t* size_tot)
{
	...
	*size_tot = 0;
	...
	while ( tail_prov != lora_head[q_id] )
	{
		...
		{
			// message trouvé
			if (compteur == cpt)  // position du message
			{
				*pos = tail_prov;
				valid = 1;
			}
