...
volatile uint16_t HDC1080_config_reg = 0;
volatile uint8_t HDC1080_read_done = 0;
uint8_t HDC1080_data_buffer[2] = {0};
...
{
    HDC1080_read_done = 0; // réinitialiser le flag
...
uint8_t HDC1080_read_tempe_humid(uint16_t* temperature, uint8_t* humidity)
{
    ...
    *temperature = 12000; // 20°C:12000 2EE0 2>>0
    *humidity = 50;    // 32
    ...
    if (ret != HAL_OK)  return 1;  // Si une erreur survient dans la lecture de la température
